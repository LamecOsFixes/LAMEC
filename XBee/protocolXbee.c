...
typedef struct
{
  ...
  uint8_t 		inbufCommandIndex;		//apontador de posi�ao do buffer
...
void Run_Frame(void)
{
  ...
  {
			if(Process_Frame(&Protocol_Machine, &tempbyte)){	//verifica se foi encontrada trama v�lida
				Analyse_Frame(&Protocol_Machine);	//tratamento de trama v�lida
			}
  }		
}
